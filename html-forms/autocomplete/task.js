class Autocomplete {
  ...
  getMatches(text) {
    const option = Array.from(document.querySelector('.autocomplete__input').options)
    let arrData = [];
    option.forEach(element => {
      arrData.push({ text: element.text, value: element.value })
    });
    let result = arrData.filter(e => e.text.includes(text))
    return result;
  }
}
